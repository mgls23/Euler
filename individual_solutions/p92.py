...
from euler.maths import operators
...
def most_optimal_answer():
	...
	def one_or_89(number):
		while number != 89 and number != 1:
			number = sum(map(operators.square, map(int, str(number))))

		return number

	last = {0: 1}
	squares = list(map(operators.square, range(10)))  # [0, 1, 4 .. 81]
	for _ in range(7):  # number of digits
		next_ = {}
		for total, result in last.items():
			for square in squares:
				number_total = total + square
				next_[number_total] = next_.get(number_total, 0) + result
